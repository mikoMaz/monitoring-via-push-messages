...
public class WebWritePreprocessor {
    ...
    JsonObject convertToJsonTreeComponent(BridgeData bridge, long status)
    {
        JsonObject device=new JsonObject();
        int deviceType=2;
        device.addProperty(f1,bridge.getSerial_number());
    ...
    JsonObject convertToJsonTreeComponent(GatewayData gateway,long status)
    {
        JsonObject device=new JsonObject();
        int deviceType=1;
       device.addProperty(f1,gateway.getGateway_eui());
    ...
    JsonObject convertToJsonTreeComponent(SensorDataSimplified sensor,long status)
    {
        JsonObject device=new JsonObject();
        int deviceType=0;
        device.addProperty(f1,sensor.getSensor());
